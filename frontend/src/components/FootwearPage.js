...
const FootwearPage = ({ 
  ...
  const categories = useMemo(() => {
    const unique = Array.from(new Set((footwearCatalog || []).map(item => item.category).filter(Boolean)));
    unique.sort((a, b) => a.localeCompare(b));
    return ['All', ...unique];
  }, []);

  const sellers = useMemo(() => {
    const unique = Array.from(new Set((footwearCatalog || []).map(item => item.seller).filter(Boolean)));
    unique.sort();
    return ['All', ...unique.slice(0, 15)];
  }, []);

  const colors = useMemo(() => {
    const unique = Array.from(new Set((footwearCatalog || []).map(item => item.color).filter(Boolean)));
    unique.sort();
    return ['All', ...unique.slice(0, 15)];
  }, []);

  const filteredProducts = useMemo(() => {
    let filtered = footwearCatalog.filter(product => Boolean(product.image));
  ...
  return (
    <div style={{ width: '100vw', position: 'relative', left: '50%', right: '50%', marginLeft: '-50vw', marginRight: '-50vw' }}>
      <PageHero
        ...
        subtitle="Step out in style with our 14 premium footwear categories. Sneakers, heels, boots, loafers & sports shoes."
        heroTag="FOOTWEAR catalog"
        tags={['Sneakers & Athletic', 'Cushioned Comfort', 'Formal & Casual']}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-8 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto justify-between">
              ...
              <select
                ...
                className="px-3 py-2 rounded-xl border border-gray-200 text-xs font-semibold text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
          ...
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
              Footwear Category Selection (14 Categories)
            </label>
        ...
        {currentProducts.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
            ...
            <h3 className="text-lg font-bold text-gray-900 mt-2">No Footwear Found</h3>
            ...
            <button
              ...
              className="mt-4 px-4 py-2 bg-blue-600 text-white font-bold text-xs rounded-xl shadow-sm hover:bg-blue-700 transition-colors"
            >
              Reset All Filters
            </button>
          </div>
